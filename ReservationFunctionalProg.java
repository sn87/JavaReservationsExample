...
public class ReservationFunctionalProg {
      ...
      public static void main(String[] args) throws ParseException {
            ...
            if (args.length == 0) {
                  ...
                  inputStrings.add(
                              "Guten Tag, einen Tisch für zwanzig Mann am 1.5. 12 Uhr mittags, Gruß Franz Schulze");
            ...
            for (String inputString : inputStrings) {
                  ...
                  try {
                        String extrctName = extractName(inputString);
                        map.put("name", extrctName);

                  } catch (Exception e) {
                        e.printStackTrace();
                  }
                  try {
                        Date extrctDate = extractDate(inputString);
                        map.put("date", extrctDate);

                  } catch (Exception e) {
                        e.printStackTrace();
                  }
                  try {
                        int extrctSeats = extractSeats(inputString);
                        map.put("seats", extrctSeats);
       ...
       * 
       * @param input the String containing the reservation and the name of the person
      ...
      public static String extractName(String input) throws Exception {
            ...
            // Alternative approach: Use NLP library from stanford (NERTagger /
            // NERClassifier) to detect human names and then extract these

            return name;
      }

      /*
       * Extracts int seats of with the amount of seats requested in the reservation
       * from String input
       * 
       * @param input the String containing the reservation and the amount of seats
       * requested in the reservation
       * 
       * @return the extracted int seats of with the amount of seats requested in the
       * reservation
       */
      public static int extractSeats(String input) throws Exception {
            int seats = 0;
            String substr = "";
       ...
       * 
       * @param input the String containing the reservation and the date of the
       * reservation
       * 
       * @return the extracted Date date of the reservation
       */
      public static Date extractDate(String input) throws Exception {
            ...
            if (m3.find()) {
                  ...
                  if (Integer.parseInt(m4.group(1)) <= 12) {
                        ...
                        if ((Integer.parseInt(m4.group(1)) >= 9 && Integer.parseInt(m4.group(1)) < 12)
                                    && input.toLowerCase().contains("nacht")) {
                              pm = true;
                        ...
                        if (Integer.parseInt(m4.group(1)) == 12) {
                              if (input.toLowerCase().contains("nachmittag") || input.toLowerCase().contains(" mittag")
                                          || input.toLowerCase().contains("abend")) {
                                    timeStr = " 12:00";
                              } else if (input.toLowerCase().contains("am morgen")
                                          || input.toLowerCase().contains("morgens")
                                          || input.toLowerCase().contains("morgends")
                                          || input.toLowerCase().contains("nacht")) {
                                    timeStr = " 00:00";
                              }
      ...
      private static int wordToInt(String word) {
            ...
            if (word.contains("und") == false) {
                  ...
                  if (kombinZahl.length() > 0)
                        wordAsInt = Integer.parseInt(kombinZahl);
            }
            return wordAsInt;
      }
}